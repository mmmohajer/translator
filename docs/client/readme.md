...
- Located in `src/baseComponents/`.
- These are reusable, low-level UI elements (e.g., Button, Input, Modal).
- Follow the pattern: one component per file, with its own SCSS module if needed.
- Example usage:
  ```jsx
...
- These are higher-level, feature-specific components (e.g., ArticleCard, UserProfile).
- Can use base components and their own styles.
...
- Store configuration is in `src/store/`.
- Slices (state modules) are in `src/reducer/`.
- To add a new state slice:
  1. Create a new file in `src/reducer/`.
  2. Define the slice using `createSlice`.
  3. Add the slice to the root reducer in `src/store/index.js`.
